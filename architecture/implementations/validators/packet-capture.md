...
Валидатор один раз проверяет канонический файл Packet Capture Next Generation (PCAPNG) `raw/target.pcapng` после его сборки и штатного закрытия. Он различает структурную корректность файла и пригодность данных для последующих стадий. Условия публикации принадлежат [стадии захвата](../../stages/10-capture/publication.md).
...
Валидатор записывает единственный отчёт по профилю `source-pcap-v1`. `validated_artifact_id` равен SHA-256 закрытых байтов канонического `raw/target.pcapng`, которые прочитаны правилами и переданы хешированию. Необязательные `raw/external.pcapng` и `raw/loopback.pcapng` не получают самостоятельных отчётов или идентификаторов валидации; сведения об их сборке могут присутствовать только как ограниченное свидетельство в `observed` соответствующего правила. Профиль содержит стабильные идентификаторы:
...
- Проверяемым артефактом всегда является `raw/target.pcapng`, а не компонент сборки.
