...
`validation.json` хранит единый машиночитаемый результат проверки артефакта. Профиль определяет набор правил для конкретного типа входа.
...
- Отчёт относится к одному явно определённому проверяемому артефакту.
