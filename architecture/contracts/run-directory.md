...
В завершённой со `status = completed` стадии 10 файл `raw/target.pcapng` обязателен. Это единственный пакетный файл стадии, который может быть входом стадии 20. `raw/external.pcapng` и `raw/loopback.pcapng` являются необязательными свидетельствами происхождения и не заменяют канонический файл.
