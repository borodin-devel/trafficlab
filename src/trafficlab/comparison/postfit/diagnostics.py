"""Typed retained fitness diagnostic records and their discriminator."""
...
class AutocorrelationDiagnostic(_DiagnosticModel):
    ...
    def validate_acf_arithmetic(self) -> Self:
        ...
        require_normalized(self.lag_weights, name="autocorrelation diagnostics.lag_weights")
        feature_weights = (self.feature_weights.iat, self.feature_weights.size)
        require_normalized(feature_weights, name="autocorrelation diagnostics.feature_weights")
...
class MultiscaleDiagnostic(_DiagnosticModel):
    ...
    scales: Annotated[tuple[MultiscaleScaleDiagnostic, ...], BeforeValidator(tuple_input)]
    ...
    def validate_scale_arithmetic(self) -> Self:
        ...
        require_normalized(self.scale_weights, name="multiscale_rate diagnostics.scale_weights")
        feature_weights = (self.feature_weights.packet, self.feature_weights.byte)
        require_normalized(feature_weights, name="multiscale_rate diagnostics.feature_weights")
        ...
        for width in self.widths:
            ...
            expected_counts.append(2 * math.ceil(snap_near_integer(quotient)))
...
class JensenShannonIatDiagnostic(StrictArtifactModel):
    ...
    categories: Annotated[tuple[JensenShannonIatCategory, ...], BeforeValidator(tuple_input)]
    jsd: UnitFloat
...
class JensenShannonMarkDiagnostic(StrictArtifactModel):
    reference_count: PositiveInt
    generated_count: PositiveInt
    categories: Annotated[tuple[JensenShannonMarkCategory, ...], BeforeValidator(tuple_input)]
    jsd: UnitFloat
...
class JensenShannonDiagnostic(_DiagnosticModel):
    ...
    def validate_js_arithmetic(self) -> Self:
        ...
        require_normalized(weights, name="jensen_shannon diagnostics.feature_weights")
        edges = self.iat.bin_edges
        if (
            len(edges) < 2
            or edges[0] != 0.0
            or any(left >= right for left, right in zip(edges, edges[1:], strict=False))
        ):
    ...
    if isinstance(value, Mapping):
        ...
        if isinstance(strata, Mapping):
            ...
            if isinstance(iat, Mapping) and isinstance(size, Mapping):
                ...
                if all(
                    type(value) is float and math.isclose(value, 1.0, rel_tol=0.0, abs_tol=WEIGHT_TOLERANCE)
