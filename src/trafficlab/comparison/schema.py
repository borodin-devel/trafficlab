...
from trafficlab.comparison.postfit.c2st_schema import (
    C2stComparison,
    C2stDiagnostic,
    C2stFoldDiagnostic,
    FanoAllanComparison,
    PostfitComparison,
    PostfitDiagnostics,
    TransitionMatrixComparison,
)
from trafficlab.comparison.postfit.schema import (
    FanoAllanDiagnostic,
    FanoAllanScaleDiagnostic,
    TransitionComponentValues,
    TransitionMatrixDiagnostic,
    TransitionOccupancyDiagnostic,
    TransitionRowDiagnostic,
    TransitionRowsDiagnostic,
    TransitionRunsDiagnostic,
...
def _smoothed_probabilities(counts: tuple[int, ...], pseudocount: float) -> tuple[float, ...]:
    """Legacy schema test seam backed by the post-fit artifact owner."""
    return smoothed_probabilities(counts, pseudocount)
...
class PublishedComparisonMethods(StrictArtifactModel):
    ...
    def items(self) -> tuple[tuple[str, PublishedMethod], ...]:
        return tuple((name, cast(PublishedMethod, getattr(self, name))) for name in FITNESS_METHOD_NAMES)
