...
def test_credential_free_fixture_round_trips_every_canonical_adverse_outcome() -> None:
    """A table that cannot be parsed offline is not reproducible failure evidence."""
    records = _fixture_records()

    outcomes = tuple(FailureOutcome.from_dict(record) for record in records)

    assert len(outcomes) == 43
    assert tuple(outcome.as_dict() for outcome in outcomes) == records
    ...
    assert all("/home/" not in line and "token" not in line.lower() and "password" not in line.lower() for line in _FIXTURE.read_text(encoding="utf-8").splitlines())
...
def test_direct_preflight_configuration_failure_carries_outcome_without_a_log(
    ...
    error = TrafficlabError("invalid run path", corrective_action="correct run.directory")
...
@pytest.mark.parametrize(
    ...
    [
        (fitting, "artifact_corrupt", "fit", "best_model.json", "not_published"),
        (generation, "generation_incomplete", "generate", "generated.pcapng", "not_published"),
